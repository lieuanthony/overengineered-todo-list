  ...
  return (
    <main style={{ flex: 1, background: bg, color: text, fontFamily: "inherit", padding: "48px 32px", display: "flex", justifyContent: "center", overflowY: "auto" }}>
      <div style={{ width: "100%", maxWidth: 480 }}>
        ...
        <form onSubmit={handleSave} style={{ background: paper, border: `1px solid ${border}`, borderRadius: 12, padding: "28px 24px", display: "flex", flexDirection: "column", gap: 16 }}>
          <div>
            <label style={{ display: "block", fontSize: 13, color: muted, marginBottom: 6 }}>Name</label>
          ...
          {success && <p style={{ margin: 0, fontSize: 13, color: accent }}>Profile updated successfully</p>}

          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button type="button" onClick={() => router.push("/dashboard")}
              style={{ padding: "10px 18px", fontSize: 14, borderRadius: 8, border: `1px solid ${border}`, background: "transparent", color: muted, cursor: "pointer", fontFamily: "inherit" }}>
              Cancel
            </button>
            <button type="submit" disabled={saving || !name.trim()} className="btn-primary"
              style={{ padding: "10px 18px", fontSize: 14, fontWeight: 500, background: accent, color: "#fff", border: "none", borderRadius: 8, cursor: saving ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: saving ? 0.7 : 1 }}>
